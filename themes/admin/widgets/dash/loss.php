                                ...
                                <form class="form-inline ajax_off mb-1" action="<?= url('/admin/dash/loss'); ?>"
                                    ...
                                    <div class="nav-search">
                                        <input type="date" class="form-control header-search mr-2" name="first_date"
                                               value="<?= $date['first_date']; ?>" placeholder="Buscar…"
                                               aria-label="Search">
                                        <input type="date" class="form-control header-search" name="second_date"
                                               value="<?= $date['second_date']; ?>" placeholder="Buscar…"
                                               aria-label="Search">
                                        <button class="btn btn-primary" type="submit"><i class="fa fa-search"></i>
                                        </button>
                                    </div>
                                </form>
                                <div class="d-flex flex-column justify-content-center align-items-end w-100 mb-2">
                                    <button class="btn-delete-negotiations btn btn-danger btn-dark d-none">
                                        <i class="fa fa-trash"></i>
                                    </button>
                                    <p class="checkboxes-checked m-0 mt-2"></p>
                                </div>
                            </div>
                                    ...
                                    <?php if ($lossArr) : ?>
                                        <?php foreach ($lossArr as $negLoss) : ?>
                                            <tr class="bg-purple text-white">
                                                <td>
                                                    <a href="<?= url('/admin/negotiations/negotiation/' . infoClientID($negLoss->client_id)->id); ?>"
                                                       class="text-white"><?= infoClientID($negLoss->client_id)->name; ?></a>
                                                </td>
                                                <td><?= infoSellerID($negLoss->seller_id) ? infoSellerID($negLoss->seller_id)->fullName() : ""; ?></td>
                                                    ...
                                                    <td>
                                                        <div class="d-flex justify-content-center">
                                                            <input type="checkbox" name="client_id"
                                                                   id="<?= infoClientID($negLoss->client_id)->id; ?>"
                                                                   class="check-form-input mr-2">

                                                            <a href="#" class="btn btn-danger btn-sm"
                                                               data-post="<?= url("/admin/clients/delete/{$negLoss->client_id}"); ?>"
                                                               data-action="delete"
                                                               data-confirm="ATENÇÃO: Tem certeza que deseja excluir a negociação relacionada ao cliente? Essa ação não pode ser desfeita!"
...
<?php $v->start("scripts"); ?>
    ...
    <script src="<?= theme('assets/js/custom/delete-negotiations.js', CONF_VIEW_ADMIN); ?>"></script>
<?php $v->end("scripts"); ?>